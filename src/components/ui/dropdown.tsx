        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div
              ...
              className="absolute z-[200] left-0 right-0 mt-2 bg-card/95 backdrop-blur-2xl border border-white/10 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] overflow-hidden py-2"
            >
              <div className="max-h-72 overflow-y-auto custom-scrollbar">
                <button
                  type="button"
                  onClick={() => { onChange(''); setIsOpen(false); }}
                  className={`w-full px-5 py-3.5 flex items-center justify-between text-[13px] font-bold transition-all hover:bg-primary/10 group ${
                    value === '' ? 'text-primary' : 'text-muted-foreground/70 hover:text-foreground'
                  }`}
                >
                  <span className="truncate">{placeholder}</span>
                  {value === '' && <Check className="h-3.5 w-3.5 stroke-[3]" />}
                </button>
                ...
                {options.map((opt) => (
                  ...
                  >
                    ...
                    {value === opt.value && <Check className="h-3.5 w-3.5 stroke-[3]" />}
                  </button>
                ))}
              </div>
            </motion.div>
